...
비어있지 않은 Leaf Node의 개수는 `Internal Node의 개수` + 1이다.
  - **증명(수학적 귀납법)**  
  ...
  - **파생 정리**  
    - 비어있지 않은 Binary Tree의 null pointer의 개수는 `leaf node의 개수 + 1`이다.
      - 증명:  
        - Binary Tree의 모든 leaf node는 null pointer를 양쪽으로 갖고 있다.
          - 따라서 null pointer를 node로 취급하면, 모든 binary tree는 leaf node를 갖는 Full Binary Tree가 된다.
        - 따라서 모든 leaf node를 internal node로 치환하고, null pointer를 leaf node로 치환하면
          - Binary Tree의 null pointer의 개수는 `leaf node의 개수 + 1`이 된다.
...
#### Preorder Traversal
- **정의**  
    노드를 방문하고 왼쪽 서브트리를 방문한 후 오른쪽 서브트리를 방문하는 방법.
...
#### Postorder Traversal
- **정의**  
    왼쪽 서브트리를 방문한 후 오른쪽 서브트리를 방문하고 마지막으로 노드를 방문하는 방법.
...
#### Inorder Traversal
- **정의**  
    왼쪽 서브트리를 방문한 후 노드를 방문하고 마지막으로 오른쪽 서브트리를 방문하는 방법.
...
- **시간 복잡도**
  ...
  - Shift Down
    ...
    - depth 1을 leaf node라 할 때, depth $i$의 노드가 shift down을 수행하면 $i-1$의 시간이 걸림
    - depth $i$의 노드 개수는 $\frac{n}{2^{i}}$
...
- **Optimality of Huffman Code**
  ...
  - 증명
    - Contradiction
      - 주장: 가장 빈도가 적은 두 문자가 root에 없을때, 최적의 External Path Weight를 가진 트리가 존재한다고 가정
      - 가정에 따라, 가장 빈도가 적은 두 문자 {A, B}가 root에 없고, 다른 문자 C가 가장 깊은 노드에 있음
      - 이 경우, A와 B를 root로 만들면 External Path Weight가 줄어들게 됨
      - 따라서 기존의 구성이 최적이 아님
      - 따라서 허프만 코드는 최적의 인코딩을 제공함.
    - Induction
      - Base case: 2개의 문자 A와 B가 있을 때, 허프만 코드는 A와 B를 root로 만들고 External Path Weight를 최소화함.
      - Inductive step:
        - 가정: n-1개의 문자가 있을 때, 허프만 코드는 External Path Weight를 최소화함.
        - 빈도가 가장 작은 두 문자를 하나로 묶어서 새로운 노드로 치환
        - 이 과정을 n → n-1 문제로 축소
        - 귀납 가정에 따라 n-1 문제의 해는 최적
        - 다시 확장해도 최적 유지됨
        - 따라 허프만 코드는 최적의 인코딩을 제공함.
