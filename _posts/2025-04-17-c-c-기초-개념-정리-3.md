...
---
## Functions
함수(Function)란 프로그램에서 특정 작업을 수행하도록 설계된 코드 블록입니다. 함수는 재사용 가능하며, 코드의 가독성과 유지보수성을 높이는 데 도움을 줍니다. 함수는 입력값(매개변수)을 받아 처리한 후 결과값(반환값)을 반환할 수 있습니다.
...
함수 선언은 함수의 이름, 반환형, 매개변수 목록을 컴파일러에 알려주는 역할을 합니다. 함수 선언은 보통 프로그램의 시작 부분이나 헤더 파일에 작성됩니다.  
...
함수 정의는 함수 선언과 일치해야 하며, 함수의 반환형과 매개변수 목록이 동일해야 합니다.


### Function Call
함수 호출은 정의된 함수를 실행하는 작업입니다. 함수 호출 시 필요한 매개변수를 전달합니다.  
...
매개변수(Parameter)는 함수 선언 및 정의에서 사용되는 변수로, 함수가 입력값을 받을 준비를 하는 역할을 합니다. 반면, 인수(Argument)는 함수 호출 시 실제로 전달되는 값입니다.

예시:
```cpp
int add(int a, int b) { // a와 b는 매개변수
    return a + b;
}

int result = add(5, 10); // 5와 10은 인수
std::cout << "Result: " << result << std::endl;
```

위 코드에서 `a`와 `b`는 함수 정의에서 사용된 매개변수이고, `5`와 `10`은 함수 호출 시 전달된 인수입니다.
...
`void` 함수는 매개변수를 가질 수도 있고, 매개변수가 없을 수도 있습니다. 매개변수를 사용하는 경우:
...
위 코드에서 `greetUser` 함수는 사용자 이름을 매개변수로 받아 출력합니다.
...
### main 함수의 매개변수
`main` 함수는 매개변수를 가질 수 있습니다. 이를 통해 명령줄 인수를 받을 수 있습니다.
...
- `argc`는 명령줄 인수의 개수를 나타냅니다.
- `argv`는 명령줄 인수의 문자열 배열입니다. `argv[0]`은 프로그램의 이름을 포함합니다.
...
위 코드는 명령줄 인수를 출력합니다. 예를 들어, 프로그램을 `./program arg1 arg2`로 실행하면 다음과 같은 출력이 나타납니다:
```
